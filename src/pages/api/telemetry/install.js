...
export default async function handler(req, res) {
  ...
  const { data: records, error: fetchError } = await supabase.from('apm_telemetry').select('ip, package');

  if (fetchError) {
    console.error('Supabase Fetch Error:', fetchError);
    return res.status(500).json({ success: false, message: 'Supabase Error during check', error: fetchError });
  }

  const alreadyExists = records?.some((r) => r.ip === ip && r.package === (packageName || 'unknown'));
